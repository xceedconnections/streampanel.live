...
if ($channel) {
    ...
    $pageTitle = "Watch {$channel_name_raw} Live TV Free - {$site_name}";
    $metaDescription = "Watch {$channel_name_raw} live TV - {$site_name}";
    // Basic keyword set using channel name + generic terms
    $metaKeywords = "{$channel_name_raw}, {$channel_name_raw} live, {$channel_name_raw} live tv, live tv, free live tv, {$site_name}";
} else {
...
<?php else: ?>
    <div class="tv-channel-page">
            ...
            <div class="sticky-header">
                ...
                <div class="mobile-header-row1">
                    ...
                    <div class="channel-info-header">
                        ...
                        <p><?php echo htmlspecialchars($channel['category'] ?? 'N/A'); ?></p>
                    </div>
                    ...
                    <?php if (!empty($sources) && count($sources) > 1 && !$showPremiumGate): ?>
                    <?php 
                    ...
                    <select id="source-selector-mobile" class="source-selector-header" style="display: none;">
                        <?php foreach ($active_sources as $index => $source): ?>
                            <?php
                                $sourceType = $source['type'] ?? 'embed';
                                $sourceUrl  = $source['url'] ?? '';
                                // For embed-style types, DO NOT put full HTML into data-url (we use iframe instead)
                                if (in_array($sourceType, ['embed', 'html-embed', 'iframe-only'], true)) {
                                    $dataUrl = '';
                                } else {
        ...
        >
            ...
            <div class="video-player-wrapper" id="video-wrapper">
                ...
                <?php
                    $is_embed_source = !empty($selected_source) && in_array($selected_source['type'] ?? '', ['embed', 'html-embed', 'iframe-only'], true);
                ...
                ?>
                <video id="videoPlayer" class="video-player" controls autoplay playsinline style="<?php echo $is_embed_source ? 'display: none;' : ''; ?>"></video>
                ...
                <div id="html-embed-container" style="<?php echo $is_embed_source ? 'display: block;' : 'display: none;'; ?> width: 100%; height: 100%;">
    ...
    <?php
        ...
        $embed_html = '';
        if (!empty($selected_source) && in_array($selected_source['type'] ?? '', ['embed', 'html-embed', 'iframe-only'], true)) {
            $embed_html = $selected_source['url'] ?? '';
        }
        $sources_for_js = array_map(function ($s) {
            if (in_array($s['type'] ?? '', ['embed', 'html-embed', 'iframe-only'], true)) {
        ...
        // For each active source, keep full embed HTML only for embed-style types; others get empty string.
    ...
    <script>
        ...
        const channelSources = <?php echo json_encode($sources_for_js, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>;
        // Full embed HTML for each active source (index-aligned with active sources / selectors), base64-encoded for safety
        const embedHtmlSourcesEncoded = <?php echo json_encode($embed_html_sources_encoded, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?>;
        ...
        let streamType = selectedSource ? selectedSource.type : 'embed';

        // For initially selected embed-style sources, prefer HTML from embedHtmlSources
        if (selectedSource && (selectedSource.type === 'embed' || selectedSource.type === 'html-embed' || selectedSource.type === 'iframe-only')) {
        ...
        function showLoadingOverlay(message = 'Connecting to stream...') {
            ...
            if (overlay && loadingText) {
                ...
                // Set timeout to show "Loading Failed" if stream doesn't start within 45 seconds
                if (loadingTimeout) {
                ...
                loadingTimeout = setTimeout(function() {
                    ...
                    if (loadingOverlayShown && overlay.style.display !== 'none' && 
                        ...
                        showLoadingError('Loading Failed - Connection timeout');
                        console.log('[TV Channel] Loading timeout - stream did not start within 45 seconds');
                    }
        ...
        function loadActualStream() {
            ...
            // Detect HTML embed source types
            const htmlEmbedContainer = document.getElementById('html-embed-container');
            const isHtmlEmbed = (
                streamType === 'embed' ||
                streamType === 'html' ||
                streamType === 'html-embed' ||
                streamType === 'iframe-only'
            );
            ...
            // If current source is not an HTML embed, make sure embed container is hidden/cleared
            if (htmlEmbedContainer && !isHtmlEmbed) {
            ...
            const isM3U = streamType === 'm3u' || streamUrl.includes('.m3u');
            ...
            console.log('[TV Channel] Player decision (HLS/DASH/native):', {
                isHLS, isDASH, isM3U, isStreamingProtocol, streamType, streamUrlSample: streamUrl.substring(0, 80)
            });
            
            // Prefer HLS.js for HLS streams
            if (isHLS && typeof Hls !== 'undefined' && Hls.isSupported()) {
                console.log('[TV Channel] Using HLS.js for HLS stream');
                hlsInstance = new Hls();
        ...
        document.addEventListener('fullscreenchange', () => {
            ...
            // Handle Android device rotation on fullscreen
        ...
        function handleSourceChange(selector) {
            ...
            selector.addEventListener('change', function() {
                ...
                const newType = selectedOption.dataset.type || 'embed';
                ...
                // For embed-style sources, load the embed in the iframe inside the container
                if (newType === 'embed' || newType === 'html-embed' || newType === 'html-embed' || newType === 'iframe-only') {
                    if (embedFrame && htmlEmbedContainer) {
