<?php
/**
 * Admin Panel - Remove Non-Working M3U8/HLS Sources
 * Checks all M3U8/HLS sources and removes only the non-working ones
 * Channels are preserved, only bad sources are removed
 */
$page_title = "Remove Bad M3U8 Sources";
...
// Function to check if M3U8 stream is working
...
if ($is_preview || $is_process) {
    ...
    foreach ($channels as $channel) {
        ...
        foreach ($sources as $source) {
            ...
            // Only check m3u8/HLS sources
            $is_m3u8 = false;
            if ($source_type === 'hls' || 
                $source_type === 'm3u8' ||
                strpos(strtolower($source_url), '.m3u8') !== false ||
                strpos(strtolower($source_url), 'm3u8') !== false ||
                strpos(strtolower($source_url), 'hls') !== false) {
                $is_m3u8 = true;
            }
    ...
    if ($is_process) {
        $message = "Cleanup complete! Checked {$stats['channels_checked']} channels, {$stats['sources_checked']} M3U8 sources. "
                 . "Removed {$stats['sources_removed']} non-working sources from {$stats['channels_updated']} channels.";
        $message_type = 'success';
    } else {
        $message = "Preview only: Found {$stats['sources_to_remove']} non-working M3U8/HLS sources across {$stats['channels_with_bad']} channel(s). "
...
<div class="bg-gray-900 rounded-lg p-6 mb-8">
    <h1 class="text-3xl font-bold mb-6">
        <i class="fas fa-link mr-2 text-netflix-red"></i>Remove Non-Working M3U8/HLS Sources
    </h1>
    
    <p class="text-gray-400 mb-6">
        This tool will check all M3U8/HLS streaming sources and remove only the non-working ones. 
    ...
    <div class="bg-gray-800 rounded-lg p-4 mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div class="text-center">
                ...
                <div class="text-sm text-gray-400">M3U8 Sources Checked</div>
            </div>
            <div class="text-center">
    ...
    <div class="bg-yellow-900 bg-opacity-30 border border-yellow-700 text-yellow-200 px-4 py-3 rounded mb-6">
        ...
        <strong>Note:</strong> This process may take several minutes depending on the number of channels. The tool will check each M3U8 source properly (HTTP status, content validation, CORS, connection errors) before removing it.
    </div>
    ...
    <div class="bg-gray-800 rounded-lg p-6">
        ...
        <form method="POST" class="mb-4">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded transition-colors">
                <i class="fas fa-search mr-2"></i>Preview Bad M3U8/HLS Sources
            </button>
        </form>

        <!-- Step 2: Run cleanup (shown after preview) -->
        <?php if ($has_run && !$is_process && $stats['sources_to_remove'] > 0): ?>
        <form method="POST" onsubmit="return confirm('This will remove all detected non-working M3U8/HLS sources. Channels will be preserved. Continue?');">
            ...
            <button type="submit" class="bg-netflix-red hover:bg-red-700 text-white font-bold py-2 px-6 rounded transition-colors">
                <i class="fas fa-play mr-2"></i>Confirm & Remove Bad M3U8 Sources
            </button>
