<?php
/**
 * Add/Edit Streaming Sources for TV Show
 */
require_once __DIR__ . '/../config/config.php';
require_once __DIR__ . '/../config/database.php';
require_once __DIR__ . '/includes/functions.php';

$page_title = "Manage TV Show Sources";
...
$show = null;
if ($show_id) {
    $show = getTVShowById($conn, $show_id);
    if ($show) {
        $show['sources'] = parseSources($show['sources'] ?? '[]');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE tv_shows SET sources = ? WHERE id = ?");
    $stmt->bind_param("si", $sourcesJson, $show_id);
    ...
    if ($stmt->execute()) {
        ...
        $message = 'Error updating sources: ' . $stmt->error;
        $message_type = 'error';
    }
}
...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6">
        ...
        <h1 class="text-4xl font-bold mb-2">Manage Streaming Sources</h1>
        ...
        <div class="bg-gray-900 rounded-lg p-6">
            ...
            <div id="sources-container" class="space-y-4">
                <?php if (!empty($show['sources'])): ?>
                    <?php foreach ($show['sources'] as $idx => $source): ?>
                        <div class="source-item bg-gray-800 rounded-lg p-4 border border-gray-700" data-index="<?php echo $idx; ?>">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <select name="sources[<?php echo $idx; ?>][quality]" class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
...
<script>
let sourceIndex = <?php echo count($show['sources'] ?? []); ?>;
    ...
    const sourceHtml = `
        <div class="source-item bg-gray-800 rounded-lg p-4 border border-gray-700" data-index="${sourceIndex}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                    <label class="block text-sm font-semibold mb-1">Quality</label>
                    <select name="sources[${sourceIndex}][quality]" class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
